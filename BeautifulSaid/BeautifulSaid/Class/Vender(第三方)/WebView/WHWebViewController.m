...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        [userContentController addScriptMessageHandler:self name:@"model"];
        
        NSString *jScript = @"var meta = document.createElement('meta'); meta.setAttribute('name', 'viewport'); meta.setAttribute('content', 'width=device-width'); document.getElementsByTagName('head')[0].appendChild(meta);";
        WKUserScript *jsString = [[WKUserScript alloc] initWithSource:jScript injectionTime:(WKUserScriptInjectionTimeAtDocumentStart) forMainFrameOnly:NO];
...
- (UIBarButtonItem *)backBarButtonItem {
    if (!_backBarButtonItem) {
        ...
        [button setImage:[UIImage imageNamed:@"btn_navigationbar_back"] forState:UIControlStateNormal];
...
- (void)setupUI{
    ...
    [self.navView.centerButton setTitle:_webTitle forState:UIControlStateNormal];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    if([message.name isEqualToString:@"model"]){
